...
from reactot.diffusion._node_dist import SingleDistributionNodes
...
def sample_qm9(
    ...
    for _ in range(n_batch):
        fragments_nodes = [
            nodes_dist.sample(bz).to(device) for _ in range(n_real)
        ]
        fragments_nodes += [
            torch.ones(bz, device=device).long() for _ in range(n_fake)
        ]
        ...
        sample_idxs = torch.cat(
            [
                ...
                torch.cumsum(fragments_nodes[0], dim=0)
            ]
        )
        for ii in range(bz):
