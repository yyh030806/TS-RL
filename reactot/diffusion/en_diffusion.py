...
from reactot.diffusion._schedule import DiffSchedule, get_repaint_schedule
...
class EnVariationalDiffusion(nn.Module):
    ...
    def __init__(
        ...
        pbc: bool = False,
        force_element_known: bool = False,  # False corresponds to legacy behavior of OA-ReactDiff
    ):
        super().__init__()
        ...
        self.pbc = pbc
        self.force_element_known = force_element_known
    ...
    def forward(
        ...
        if t_in is not None:
            t_int = torch.ones_like(t_int, device=device).float() * t_in
        ...
        if self.pos_only and (force_element_known or self.force_element_known):  # Heck
            z_t = [
                torch.cat([
                    z_t[ii][:, : self.pos_dim], xh[ii][:, self.pos_dim:]
                ], dim=1) for ii in range(len(xh))
            ]
    ...
    def log_pxh_given_z0_without_constants(
        ...
        z_t = [_z_t[:, : self.node_nfs[0]] for _z_t in z_t]
    ...
    def sample(
        ...
        for s in reversed(range(0, timesteps)):
            ...
            if torch.max(torch.abs(zt_xh[0])) > 1e5:
                raise ValueError("scroing network gives insane predictions...")
